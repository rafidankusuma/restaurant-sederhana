            ...
            {reservation.map((item) => (
                <div className="bg-white shadow pb-4 mb-4 md:pb-0 relative" key={item.id}>
                    ...
                    <div className="flex flex-col mb-4 items-start bg-white rounded-sm md:flex-row md:w-full">
                        <Image src={item.Food.image} width={500} height={300} className="object-cover w-full rounded-t-sm 
                        h-60 md:h-auto md:w-1/3 md:rounded-none md:rounded-s-sm" alt="image food" />
                                ...
                                <div className="flex items-center justify-between text-sm font-medium text-gray-900 truncate">
                                    ...
                                    <span>
                                        {Array.isArray(item.Food)
                                            ? item.Food.reduce((total, food) => total + (food.quantity || 1), 0)
                                            : 1
                                        }
                                    </span>
                                </div>  
                    ...
                    <div className="flex items-end justify-end absolute inset-4">
                        {item.Payment?.status === "unpaid" ? (
                            ...
                            rounded-md hover:bg-orange-500">Pay Now</Link>
                        ) : (
                            <Link href={`/myorder/${item.id}`} className="px-5 py-1 bg-orange-400 text-white 
                            rounded-md hover:bg-orange-500">View Detail</Link>
                        )}
                    </div>
                </div>
            ))}
